...
with open(os.path.join(os.path.dirname(__file__), "logo_b64.txt")) as _f:
    LOGO_DATA_URI = "data:image/jpeg;base64," + _f.read().strip()
...
def login():
    ...
    return render_template_string(LOGIN_HTML, logo=LOGO_DATA_URI)
...
def index():
    ...
    return render_template("index.html", apps=apps_list, username=user_name, logo=LOGO_DATA_URI)
...
def find_main_file(extract_dir):
    """Search top-level first, then recursively, for a runnable entry file."""
    candidates = ["main.py", "bot.py", "app.py", "index.js", "server.js"]
    for f in candidates:
        if os.path.exists(os.path.join(extract_dir, f)):
            return extract_dir, f
...
def ensure_dependencies(run_dir, log_file):
    """Auto-install requirements.txt / package.json if present, so projects don't
    crash immediately with ImportError / Cannot find module."""
    req_path = os.path.join(run_dir, "requirements.txt")
    if os.path.exists(req_path):
        log_file.write("[hosting] requirements.txt পাওয়া গেছে, install হচ্ছে...\n")
        log_file.flush()
        subprocess.run(
            ["pip", "install", "--no-cache-dir", "-r", req_path],
            cwd=run_dir, stdout=log_file, stderr=log_file
        )
    pkg_path = os.path.join(run_dir, "package.json")
    if os.path.exists(pkg_path) and not os.path.exists(os.path.join(run_dir, "node_modules")):
        log_file.write("[hosting] package.json পাওয়া গেছে, npm install হচ্ছে...\n")
        log_file.flush()
        subprocess.run(["npm", "install"], cwd=run_dir, stdout=log_file, stderr=log_file)
...
def run(name):
    ...
    if (user_name, name) not in processes or processes[(user_name, name)].poll() is not None:
        run_dir, main_file = find_main_file(extract_dir)
        log_path = os.path.join(app_dir, "logs.txt")
        log_file = open(log_path, "a")
        if not main_file:
            log_file.write("[hosting] ERROR: কোনো main.py / bot.py / app.py / index.js / server.js ফাইল পাওয়া যায়নি (extracted ফোল্ডারে বা তার ভেতরের সাবফোল্ডারে)।\n")
            log_file.close()
            return redirect(url_for("index"))
        try:
            ensure_dependencies(run_dir, log_file)
            cmd = ["python", main_file] if main_file.endswith('.py') else ["node", main_file]
            processes[(user_name, name)] = subprocess.Popen(cmd, cwd=run_dir, stdout=log_file, stderr=log_file, text=True)
            db = load_db()
            db["start_times"][f"{user_name}_{name}"] = int(time.time() * 1000)
            save_db(db)
        except Exception as e:
            log_file.write(f"[hosting] ERROR: প্রসেস স্টার্ট করা যায়নি: {e}\n")
            log_file.close()
    return redirect(url_for("index"))
